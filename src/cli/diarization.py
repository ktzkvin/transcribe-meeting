...
def main(conf_path :str = "config/diarization.json", _id : str =str(uuid4())):
    ...
    segmentation = SegmentationModel.from_pyannote(audio_config.segmentation_model.value)
    ...
    if audio_config.audio_path:
        audio = FileAudioSource(file=audio_config.audio_path, sample_rate=audio_config.sample_rate)
    else:
        audio = MicrophoneAudioSource(device=audio_config.mic)
    config = SpeakerDiarizationConfig(segmentation=segmentation, 
                                      embedding=embedding, 
